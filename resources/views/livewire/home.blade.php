        ...
        <div class="px-4 py-16 mx-auto max-w-7xl md:py-24">
            <div class="max-w-2xl p-8 mx-auto text-center shadow-xl rounded-3xl bg-white/70 backdrop-blur">
                ...
                <div class="flex items-center justify-center gap-3 mt-6">
                    <a href="#catalog" class="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-white transition rounded-2xl bg-slate-900 hover:bg-slate-800">
                        Shop collection
                    </a>
                    <a href="#about" class="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium transition bg-white border rounded-2xl border-slate-300 text-slate-900 hover:bg-slate-50">
                        Learn more
                    </a>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 md:flex-shrink-0">
                ...
                <div class="relative flex-1 md:w-64">
                    ...
                    <input
                        ...
                        class="w-full py-2 pr-4 text-sm border rounded-full border-slate-300 pl-9 focus:border-slate-900 focus:ring-1 focus:ring-slate-900"
                    />
                </div>
                ...
                <select
                    wire:model="sort"
                    class="px-4 py-2 text-sm border rounded-full border-slate-300 focus:border-slate-900 focus:ring-1 focus:ring-slate-900"
                >
    ...
    <section id="catalog" class="px-4 py-10 mx-auto max-w-7xl md:px-6">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @forelse($this->products as $product)
                <div class="overflow-hidden bg-white border-0 shadow-sm group rounded-3xl ring-1 ring-slate-100">
                    <!-- Product Image -->
                    <div class="relative p-0">
                        <a href="{{ route('product.view', $product->defaultUrl->slug) }}">
                            @if ($product->thumbnail)
                                ...
                                />
                            @else
                                <div class="flex items-center justify-center w-full h-64 bg-slate-200">
                        ...
                        @if ($this->saleCollection && $product->collections->contains($this->saleCollection))
                            <div class="absolute left-3 top-3">
                                <span class="px-3 py-1 text-xs font-medium text-white rounded-full bg-slate-900">sale</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between p-5 pt-0">
                        ...
                        <a
                            ...
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition rounded-full bg-slate-900 hover:bg-slate-800"
                        >
                            View
                        </a>
                    </div>
                ...
                <div>
                    ...
                    >
                        <input
                            ...
                            class="flex-1 px-4 py-2 border rounded-2xl border-slate-300 focus:border-slate-900 focus:ring-1 focus:ring-slate-900"
                        />
                        <button
                            type="submit"
                            class="px-6 py-2 text-sm font-medium text-white transition rounded-2xl bg-slate-900 hover:bg-slate-800 whitespace-nowrap"
                        >
                            Subscribe
                        </button>
                    </form>
