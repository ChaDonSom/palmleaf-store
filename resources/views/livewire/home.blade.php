    ...
    <section class="bg-center bg-cover border-b" style="background-image: url('https://images.unsplash.com/photo-1557308970-df80a9ccee84?q=80&w=1600&auto=format&fit=crop');">
        <div class="px-4 py-16 mx-auto max-w-7xl md:py-24">
            <div class="max-w-2xl p-8 mx-auto text-center shadow-xl rounded-3xl bg-white/70 backdrop-blur">
                <div class="inline-flex items-center gap-2 px-3 py-1 mx-auto mb-3 text-xs bg-white border rounded-full text-blue-600">
                    <x-icons.sparkles />
                ...
                <p class="mt-3 text-blue-600 md:text-lg">
                    Thoughtfully designed, soft‑to‑live‑in apparel that whispers the Gospel in everyday moments.
                </p>
                <div class="flex items-center justify-center gap-3 mt-6">
                    <a href="#catalog" class="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-white transition rounded-2xl bg-blue-400 hover:bg-blue-500">
                        Shop collection
                    </a>
                    <a href="#about" class="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium transition bg-white border rounded-2xl border-blue-300 text-slate-900 hover:bg-blue-50">
                        Learn more
                    </a>
                </div>
            </div>
        ...
        <div class="flex flex-col items-stretch gap-3 px-4 py-4 mx-auto max-w-7xl md:flex-row md:items-center md:justify-between md:px-6">
            ...
            <div class="flex flex-wrap w-full gap-2">
                @foreach ($this->categories as $cat)
                    <button
                        ...
                        class="rounded-full border px-4 py-2 text-sm transition {{ $category === $cat ? 'border-blue-400 bg-blue-400 text-white' : 'bg-white text-slate-900 hover:border-blue-300' }}"
                    >
                        {{ $cat }}
                    </button>
                @endforeach
            ...
            <div class="flex items-center gap-2 md:flex-shrink-0">
                ...
                <div class="relative flex-1 md:w-64">
                    <x-icons.search class="absolute -translate-y-1/2 left-3 top-1/2 text-blue-400" />
                    <input
                        type="text"
                        wire:model.debounce.500ms="query"
                        placeholder="Search products"
                        class="w-full py-2 pr-4 text-sm border rounded-full border-blue-300 pl-9 focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
                    />
                </div>
                ...
                <select
                    wire:model="sort"
                    class="px-4 py-2 text-sm border rounded-full border-blue-300 focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
                >
    ...
    <section id="catalog" class="px-4 py-10 mx-auto max-w-7xl md:px-6">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @forelse($this->products as $product)
                <div class="overflow-hidden bg-white border-0 shadow-sm group rounded-3xl ring-1 ring-blue-100">
                    <!-- Product Image -->
                    <div class="relative p-0">
                        <a href="{{ route('product.view', $product->defaultUrl->slug) }}">
                            @if ($product->thumbnail)
                                ...
                                />
                            @else
                                <div class="flex items-center justify-center w-full h-64 bg-blue-100">
                        ...
                        @if ($this->saleCollection && $product->collections->contains($this->saleCollection))
                            <div class="absolute left-3 top-3">
                                <span class="px-3 py-1 text-xs font-medium text-white rounded-full bg-blue-400">sale</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="p-5">
                        <h3 class="text-lg font-semibold">
                            <a href="{{ route('product.view', $product->defaultUrl->slug) }}" class="hover:text-blue-600">
                                {{ $product->translateAttribute('name') }}
                            </a>
                        </h3>
                        <p class="mt-1 text-sm text-blue-600 line-clamp-2">
                    ...
                    <div class="flex items-center justify-between p-5 pt-0">
                        ...
                        <a
                            ...
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition rounded-full bg-blue-400 hover:bg-blue-500"
                        >
                            View
                        </a>
                    </div>
            ...
            @empty
                <div class="py-12 text-center col-span-full">
                    <p class="text-blue-500">No products found matching your criteria.</p>
                </div>
            @endforelse
        </div>
    </section>
    ...
    <section class="bg-white border-y">
        <div class="grid grid-cols-1 gap-6 px-4 py-10 mx-auto max-w-7xl md:grid-cols-3 md:px-6">
            <div class="p-6 border shadow-sm rounded-3xl">
                ...
                <div class="mt-1 text-blue-600">We source cozy, durable garments you'll reach for daily.</div>
            </div>
            <div class="p-6 border shadow-sm rounded-3xl">
                <div class="text-lg font-bold">DTF + Embroidery Craft</div>
                <div class="mt-1 text-blue-600">Crisp prints and stitched details that last with care.</div>
            </div>
            <div class="p-6 border shadow-sm rounded-3xl">
                <div class="text-lg font-bold">Small‑Batch, Heart‑Led</div>
                <div class="mt-1 text-blue-600">Designed near High Point & Asheboro, NC.</div>
            </div>
        </div>
    </section>
        ...
        <div class="px-4 mx-auto max-w-7xl py-14 md:px-6">
            <div class="grid items-center gap-8 p-4 shadow-xl rounded-3xl bg-white/70 sm:p-8 backdrop-blur md:grid-cols-2">
                <div>
                    ...
                    <p class="mt-1 text-blue-600">Drops, restocks, and pop‑up markets—straight to your inbox.</p>
                    ...
                    >
                        <input
                            ...
                            class="flex-1 px-4 py-2 border rounded-2xl border-blue-300 focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
                        />
                        <button
                            type="submit"
                            class="px-6 py-2 text-sm font-medium text-white transition rounded-2xl bg-blue-400 hover:bg-blue-500 whitespace-nowrap"
                        >
                            Subscribe
                        </button>
                    </form>
                    <p class="mt-2 text-xs text-blue-500">We respect your privacy. Unsubscribe anytime.</p>
                </div>
