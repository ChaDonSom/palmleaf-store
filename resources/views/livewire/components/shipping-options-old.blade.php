<form wire:submit.prevent="save" class="border rounded shadow-lg">
        ...
        <div class="p-4 border-t">
            @foreach($this->shippingOptions as $option)
                <label class="flex items-center w-full cursor-pointer">
                    <input type="radio" wire:model.live="chosenOption" value="{{ $option->getIdentifier() }}" />
    ...
    <div class="flex justify-end w-full p-4 bg-gray-100">
        <div>
            <button type="submit" wire:key="submit_btn" class="px-5 py-3 font-medium text-white bg-slate-700 rounded-3xl hover:bg-slate-600">
                Continue
            </button>
        </div>
    </div>
